    ...
    public partial class frmEditarOfertaTrabajo : System.Web.UI.Page
    {
        ...
        private void cargarDatosOfertaTrabajo()
        {
            if (Session["detalles_oferta"] !=null)
            {
                ...
                HttpContext.Current.Session["id_empresa"] = TablaOfertaTrabajo.Rows[0]["Id_OfertaTrabajo"];
        ...
        protected void dgRequisitos_ItemCommand(object source, DataGridCommandEventArgs e)
        {
            ...
            {
                object id_req_obj =((DataTable)HttpContext.Current.Session["oferta_requisitos"]).Rows[e.Item.ItemIndex][0];
                try
                {
                ...
                {
                    ((ArrayList)HttpContext.Current.Session["requisitos_agregar"]).Remove(((DataTable)HttpContext.Current.Session["oferta_requisitos"]).Rows[e.Item.ItemIndex][1]);
                }
                ((DataTable)HttpContext.Current.Session["oferta_requisitos"]).Rows.RemoveAt(e.Item.ItemIndex);
                dgRequisitos.DataSource = ((DataTable)HttpContext.Current.Session["oferta_requisitos"]);
                dgRequisitos.DataBind();
            }
        }
